  ...
  const handleInitiatePayment = async () => {
    try {
      ...
      if (response.success && response.paymentData) {
        // Store the order ID in localStorage before redirecting to eSewa
        // This will help us check the status later if the redirect doesn't have params
        localStorage.setItem('lastEsewaOrderId', orderId);
        localStorage.setItem('esewaPaymentInitiated', new Date().toISOString());
        
        // Add information about purchased items and whether this is a direct purchase
        // These flags will be read by the success page
        const isDirectPurchase = !!localStorage.getItem('isDirectPurchase');
        if (isDirectPurchase) {
          // Ensure we have the purchased items stored in localStorage
          if (!localStorage.getItem('purchasedItems')) {
            const purchasedItems = response.paymentData.purchasedItems || 
                                  [response.paymentData.transaction_uuid];
