...
import PaymentCompletionHandler from '../../components/payments/PaymentCompletionHandler';
...
const EsewaSuccess = () => {
  ...
  // Check if we have a stored order ID
  useEffect(() => {
  ...
  useEffect(() => {
    if (orderStatus && storedOrderId && Object.keys(new URLSearchParams(location.search)).length === 0) {
      ...
      if (orderStatus.success) {
        setPaymentSuccessful(true); // Mark payment as successful to clear cart
        Swal.fire({
          icon: 'success',
  ...
  const verifyPaymentWithBackend = async (paymentResponse) => {
    try {
      ...
      // We'll still assume success and clear the cart for a better user experience
  ...
  const showGenericSuccessMessage = () => {
    // Assuming payment is successful since we're on success page
    setPaymentSuccessful(true);
    Swal.fire({
      icon: 'success',
      title: 'Payment Received',
  ...
  const currentOrderId = paymentDetails?.transaction_uuid || storedOrderId;
  
  // Check if this was a direct purchase (not from cart)
  const isDirectPurchase = !!(localStorage.getItem('isDirectPurchase') === 'true');
  
  // Get purchased item IDs (if direct purchase)
  const purchasedItemIds = isDirectPurchase ? 
  ...
  return (
    <>
      {/* Handle cart updates after successful payment */}
