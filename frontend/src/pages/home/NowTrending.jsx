...
const NowTrending = () => {
  const [trendingbooks, setBooks] = useState([]);

  useEffect(() => {
    const fetchBooks = async () => {
      try {
        const response = await fetch("trendingbooks.json");
        if (!response.ok) throw new Error("Failed to load books");
        const data = await response.json();
        
        if (!data.trending_books || !Array.isArray(data.trending_books)) {
  ...
  return (
    <div className="container mx-auto px-6 py-16 bg-white-500">
      <div className="mb-12 text-left">
        ...
        <p className="text-lg text-gray-600 mt-2">Discover what others are reading</p>
      </div>

      {trendingbooks.length === 0 ? (
        <div className="text-center py-12 bg-white rounded-xl shadow-sm">
          <p className="text-gray-500 text-lg">No trending books available currently.</p>
        </div>
      ) : (
        <Swiper
          spaceBetween={30}
        ...
        >
          ...
          {trendingbooks.map((book) => (
            <SwiperSlide key={book.id} className="pb-12 hover:-translate-y-2 transition-all duration-300">
              <BookCard book={book} />
            </SwiperSlide>
          ))}
        </Swiper>
